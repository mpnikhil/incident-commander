...
describe('fixtures/timestamp', async () => {
    it('inserts a current timestamp column at the present timestamp', async () => {
        ...
        await db
            ...
            .execute();
        const { createdAtNowLocaltime, updatedAtNowLocaltime, timediffNowLocaltime, createdAtNowUTC, updatedAtNowUTC, timediffNowUTC, createdAtCurrentTimestamp, updatedAtCurrentTimestamp, timediffCurrentTimestamp, createdAtStrftime, updatedAtStrftime, timediffStrftime, } = await db
            .selectFrom('Example')
            .select(({ fn }) => [
            'createdAtNowLocaltime',
            'updatedAtNowLocaltime',
            fn('timediff', ['createdAtNowLocaltime', 'updatedAtNowLocaltime']).as('timediffNowLocaltime'),
            'createdAtNowUTC',
            'updatedAtNowUTC',
            fn('timediff', ['createdAtNowUTC', 'updatedAtNowUTC']).as('timediffNowUTC'),
            'createdAtCurrentTimestamp',
            'updatedAtCurrentTimestamp',
            fn('timediff', ['createdAtCurrentTimestamp', 'updatedAtCurrentTimestamp']).as('timediffCurrentTimestamp'),
            'createdAtStrftime',
            'updatedAtStrftime',
            fn('timediff', ['createdAtStrftime', 'updatedAtStrftime']).as('timediffStrftime'),
        ])
            .where('exampleId', '=', '1')
        ...
        const deltaCurrentTimestamp = Math.abs(Date.parse(updatedAtCurrentTimestamp) - Date.parse(createdAtCurrentTimestamp));
