  ...
  async fetch(request) {
    ...
    app.post("/incidents", async (c) => {
      ...
      try {
        ...
        await this.env.INCIDENTS_DB.prepare(`INSERT INTO incidents (id, status, title, description, severity, trace_id, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(
          incident.id,
          incident.status,
          incident.title,
    ...
    app.get("/incidents/:id", async (c) => {
      ...
      try {
        const incident = await this.env.INCIDENTS_DB.prepare("SELECT * FROM incidents WHERE id = ?").bind(incidentId).first();
        if (!incident) {
    ...
    app.get("/incidents", async (c) => {
      ...
      try {
        const result = await this.env.INCIDENTS_DB.prepare("SELECT * FROM incidents ORDER BY created_at DESC LIMIT 50").all();
